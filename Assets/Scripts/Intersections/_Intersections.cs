    ...
    public static class _Intersections
    {
        ...
        public static bool PointPolygon(List<Vector2> polygonPoints, Vector2 point)
        {
            ...
            for (int i = 0; i < polygonPoints.Count; i++)
            {
                ...
                if (_Intersections.LineLine(new Edge2(l1_p1.ToMyVector2(), l1_p2.ToMyVector2()), new Edge2(l2_p1.ToMyVector2(), l2_p2.ToMyVector2()), includeEndPoints: true))
                {
        ...
        //http://thirdpartyninjas.com/blog/2008/10/07/line-segment-intersection/
        public static Vector2? AreLinesIntersecting(Vector2 l1_p1, Vector2 l1_p2, Vector2 l2_p1, Vector2 l2_p2, bool shouldIncludeEndPoints)
        {
            float denominator = (l2_p2.y - l2_p1.y) * (l1_p2.x - l1_p1.x) - (l2_p2.x - l2_p1.x) * (l1_p2.y - l1_p1.y);
            ...
            if (denominator != 0f)
            {
                ...
                if (shouldIncludeEndPoints)
                {
                    ...
                    if (u_a >= 0f && u_a <= 1f && u_b >= 0f && u_b <= 1f)
                        return (GetLineLineIntersectionPoint(l1_p1, l1_p2, l2_p1, l2_p2));
                }
                else
                {
        ...
        // Función que devuelve true si el punto 'pointToCheck' está en la línea formada por los puntos 'point1' y 'point2'
        public static bool IsPointOnLine(Vector2 point1, Vector2 point2, Vector2 pointToCheck, float tolerance)
        {
            // Calcula la distancia entre 'point1' y 'pointToCheck', y entre 'point2' y 'pointToCheck'
            float distance1ToCheck = Vector2.Distance(point1, pointToCheck);
            float distance2ToCheck = Vector2.Distance(point2, pointToCheck);

            // Calcula la distancia entre 'point1' y 'point2'
            float distance1To2 = Vector2.Distance(point1, point2);

            // Comprueba si la suma de las distancias entre 'point1' y 'pointToCheck', y entre 'point2' y 'pointToCheck'
            // es aproximadamente igual a la distancia entre 'point1' y 'point2'
            return Mathf.Abs(distance1ToCheck + distance2ToCheck - distance1To2) < tolerance;
        }
        
        //The list describing the polygon has to be sorted either clockwise or counter-clockwise because we have to identify its edges
        public static bool IsPointInPolygon(List<Vector2> vs, Vector2 point, bool borderIsOutside = true)
        {
            int crossingCount = 0;
            ...
            for (int i = 0, j = vs.Count - 1; i < vs.Count; j = i++)
            {
                ...
                {
                    float intersectionX = (vs[j].x - vs[i].x) * (point.y - vs[i].y) / (vs[j].y - vs[i].y) + vs[i].x;
                    
                    if (Math.Abs(point.x - intersectionX) < 0.0001f && borderIsOutside == false)
                    {
                        return true;
                    }
                    else if (point.x < intersectionX)
                    {
                        crossingCount++;
                    }
                }
        ...
        public static Vector4? IsPointInPolygonBorder(List<Vector2> vs, Vector2 point, float tolerance = 0.001f)
        {
            for (int i = 0, j = vs.Count - 1; i < vs.Count; j = i++)
            {
                if ((vs[i].y > point.y) != (vs[j].y > point.y))
                {
                    ...
                    if (Math.Abs(point.x - intersectionX) < tolerance)
                    {
                        return new Vector4(vs[i].x, vs[i].y, vs[j].x, vs[j].y);
                    }
                }
            }
            return null;
        ...
        public static bool IsPointInPolyline(List<Vector3> polyline, Vector3 point, float tolerance = 0.001f)
        {
            for (var index = 0; index < polyline.Count-1; index++)
            {
                var p1 = polyline[index];
                var p2 = polyline[index+1];
                if (IsPointOnLine(p1.XZ(), p2.XZ(), point.XZ(), tolerance))
                {
                    return true;
                }
            }
        ...
        //The list describing the polygon has to be sorted either clockwise or counter-clockwise because we have to identify its edges
        public static Vector2? IsLineIntersectingPolygon(List<Vector2> polygonPoints, Vector2 point1, Vector2 point2)
        {
            ...
            for (int i = 0; i < polygonPoints.Count; i++)
            {
                ...
                Vector2? colision = null;
                
                if (point2 != null)
                    colision = AreLinesIntersecting(point1, point2, l2_p1, l2_p2, true);
                
                if (colision != null)
                {
                    Vector2 c = colision.Value;
                    //Debug.Log("Colision -> " + c);
                    return c;
                }
            }

            bool p1Inside = IsPointInPolygon(polygonPoints, point1);
            bool p2Inside = IsPointInPolygon(polygonPoints, point2);

            /*if (p1Inside && !p2Inside)
            {
                Debug.LogWarning("Ojo, no ha detectado colisión, pero sí ha detectado que se ha salido del polígono \n Point 1: " + point1 + " \n Point 2: " + point2);
            }*/
                    
            return null;
        }
        public static List<Vector2> GetPolygonIntersectionsWithPolygon(List<Vector2> polygonPoints1, List<Vector2> polygonPoints2, bool closeShape1, bool closeShape2)
        {
            var auxIndices = new List<int>();
            return GetPolygonIntersectionsWithPolygon(polygonPoints1, polygonPoints2, closeShape1, closeShape2, out auxIndices);
        }
        public static List<Vector2> GetPolygonIntersectionsWithPolygon(List<Vector2> polygonPoints1, List<Vector2> polygonPoints2, bool closeShape1, bool closeShape2, out List<int> indices)
        {
            List<Vector2> collisions = new List<Vector2>();

            indices = new List<int>();
            
            for (int i = 0; i < polygonPoints1.Count; i++)
            {
                //Line 2
                Vector2 l2_p1 = polygonPoints1[i];
                int iPlusOne = i + 1;
                if(closeShape1)
                    iPlusOne = MathUtility.ClampListIndex(i + 1, polygonPoints1.Count);
                else if(i + 1 >= polygonPoints1.Count)
                        continue;
                Vector2 l2_p2 = polygonPoints1[iPlusOne];
                List<Vector2> colisionVector2s = GetLineIntersectionsWithPolygon(polygonPoints2, l2_p1, l2_p2, closeShape2);
                
                if (colisionVector2s != null && colisionVector2s.Count > 0)
                {
                    collisions.AddRange(colisionVector2s);
                    for (var index = 0; index < colisionVector2s.Count; index++)
                    {
                        indices.Add(iPlusOne);
                    }
        ...
        public static List<Vector2> GetLineIntersectionsWithPolygon(List<Vector2> polygonPoints, Vector2 point, Vector2? lastPoint, bool closeShape = true)
        {
            List<Vector2> collisions = new List<Vector2>();
            ...
            for (int i = 0; i < polygonPoints.Count; i++)
            {
                ...
                if (!closeShape)
                    iPlusOne = i + 1;
                if(iPlusOne >= polygonPoints.Count)
                    continue;
                Vector2 l2_p2 = polygonPoints[iPlusOne];
                ...
                if (lastPoint != null)
                    colision = AreLinesIntersecting(point, lastPoint.Value, l2_p1, l2_p2, true);
                
                if (colision != null)
                {
                    Vector2 c = colision.Value;
                    collisions.Add(new Vector2(c.x, c.y));
                }
            }
