...
namespace Habrador_Computational_Geometry
{
    ...
    public static class DelaunayIncrementalSloan
    {
        public static HalfEdgeData2 GenerateTriangulation(HashSet<MyVector2> points, HalfEdgeData2 triangulationData)
        {
            ...
            if (points.Count < 2)
            {
                Debug.Log("Can't make a delaunay with sloan with less than 2 points");
            ...
            {
                //Step 5-7
                if (!InsertNewPointInTriangulation(p, triangulationData, ref missedPoints, ref flippedEdges))
                {
        ...
        public static bool InsertNewPointInTriangulation(MyVector2 p, HalfEdgeData2 triangulationData, ref int missedPoints, ref int flippedEdges)
        {
            ...
            if (!AddTrianglesOppositePToStack(p, trianglesToInvestigate, triangulationData))
            {
                return false;
            }
        ...
        private static bool AddTrianglesOppositePToStack(MyVector2 p, Stack<HalfEdge2> trianglesOppositeP, HalfEdgeData2 triangulationData)
        {
